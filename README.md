...
并行回收：同步问题
主体并发回收：用户程序和GC的竞争问题
主体并发增量回收：还是有STW，通知到所有线程

有几个 GC 的阶段，开始的时候给所有 p 一个 gcBgMarkWorker 协程，先 STW 通知开始标记（开启写屏障，允许GC标志工作），然后再 STW 终止 Mark 阶段（关闭写屏障，关闭 GC 标志工作）进入清扫阶段，进入 GCOFF 阶段，之前的标记为黑色，之后的标记为白色
